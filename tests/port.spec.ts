...
describe('端口对象', () => {
  describe('在收到远程端口发送过来的消息时', () => {
    ...
    let port: Port
    let onMessageCb: IMessageCb

    beforeEach(() => {
      // @ts-ignore
      chromePort = new chrome.__types.Port()
      spyOn(
        chromePort.onMessage,
        'addListener'
    ...
    describe('如果远程端口传过来的消息有 id', () => {
      describe('且此 id 不在本地端口的待响应列表里', () => {
        it('当消息 resolved 时会发送响应', done => {
          ...
          ;(port.emit as jasmine.Spy).and.callFake(
            (
              name: string,
              data: any,
              resolve: IPromiseFn,
  ...
  describe('如果端口在发送了需要响应的消息后连接被断开', () => {
    ...
    it('则会通知待响应列表里的每一个回调函数', () => {
      ...
      expect(s).toHaveBeenCalledWith(
        new Error('Connection has been disconnected by yourself.')
      )
      // @ts-ignore
    ...
    it('若连接是由远程端口断开的则变更错误消息', () => {
      ...
      expect(s).toHaveBeenCalledWith(
        new Error('Connection has been disconnected by the other side.')
      )
    })
  })
  ...
  describe('发送消息时', () => {
    ...
    describe('若消息需要响应', () => {
      ...
      it('会在待响应列表里注册一个函数', done => {
        port.send('x', 'y', true)!.then(response => {
      ...
      it('会在待响应列表里注册一个函数2', done => {
        port.send('x', 'y', true)!.catch(error => {
